...
- Preserves file line endings and trailing whitespace/comments.
...
# Match: optional decorators, then 'var ... := ...' on a single line.
...
def split_lines_keepends(raw: bytes) -> List[bytes]:
    ...
    # Python's splitlines(keepends=True) will preserve eols per line
...
def process_line(line: bytes, include_typed: bool, extra_tokens: Iterable[str]) -> Tuple[bytes, bool]:
    ...
    new_text = f"{indent}{decorators}var {lhs} = {rhs}{eol}"
...
def git_list_files(mode: str) -> List[str]:
    ...
    if res.returncode != 0:
        return []
    files = [ln.strip() for ln in res.stdout.splitlines() if ln.strip().endswith(".gd")]
    return files


def collect_gd_files(root: str, excludes: Sequence[str], scope: str | None) -> List[str]:
    if scope in ("tracked", "staged", "changed"):
        files = git_list_files({"tracked": "tracked", "staged": "staged", "changed": "changed"}[scope])
        if not files:
            return []
        if root != ".":
            # Filter to those under root
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        if should_exclude(dirpath, excludes):
            continue
        # in-place prune to avoid descending into excluded children
...
def scan(paths: List[str], write: bool, report: bool, include_typed: bool, extra_tokens: List[str], backup: bool, confirm: bool) -> Tuple[int, int]:
    total_hits = 0
    total_changes = 0
    for fp in paths:
        try:
        ...
        if b":=" not in raw:
            continue
        lines = split_lines_keepends(raw)
        ...
        if report and file_hits:
            for ln in changed_lines:
                # print the original textual line for context
                try:
...
def main(argv: List[str]) -> int:
    ...
    hits, changes = scan(files, write=args.write, report=report, include_typed=args.include_typed, extra_tokens=args.extra_token, backup=args.backup, confirm=args.confirm)
