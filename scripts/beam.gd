...
func activate(pos: Vector2, dir: Vector2, dmg: int, col: Color, length: float = -1.0) -> void:
    ...
    if line:
        line.default_color = color
    var len: float = max_length if length <= 0.0 else length
    # Raycast to first collider to place beam end and apply damage.
    var space: PhysicsDirectSpaceState2D = get_world_2d().direct_space_state
    var to: Vector2 = pos + dir.normalized() * len
...
func _set_line(from_local: Vector2, to_local: Vector2) -> void:
    if not line:
        return
    line.clear_points()
    line.add_point(from_local)
    line.add_point(to_local)
