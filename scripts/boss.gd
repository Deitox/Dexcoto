...
func _apply_wave_scaling() -> void:
    ...
    var hp := int(round(float(base_max_health) * pow(1.35, float((w - 1) / 5))))
    var dmg := int(round(float(base_contact_damage) * (1.0 + 0.12 * float((w - 1) / 5))))
    base_max_health = max(1, hp)
    base_contact_damage = max(1, dmg)
    # Size and color cue
    var s: float = 1.6 + 0.05 * float((w - 1) / 5)
    scale = Vector2(s, s)
    if poly:
